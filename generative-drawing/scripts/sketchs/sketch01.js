...
var vertices, loopCount, c, img, hs, btn;
...
function setup(){
  ...
  btn = createButton('redessiner',1);
  btn.mousePressed(redessine);
...
function generateRandomGaussianPoints(nb){
  if(hs.length){
    ...
    for(var i=0; i<nb; i++){
      var a,b;
      a = (random(0,1) > 0.5) ? -1 : 1;
      b = (random(0,1) > 0.5) ? 1 : -1;
      if(i > cLimit ) {
        cLimit += mod;
        c++;
      }
      tab.push([ randomGaussian(hs[c][0], i/(c+1)), randomGaussian(hs[c][1], i/(c+1)) ]);
    }
    return tab;
  }else{
